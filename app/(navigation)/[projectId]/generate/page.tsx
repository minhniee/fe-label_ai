...
export default function ProjectGeneratePage() {
  ...
  const buildGeneratedFileName = (baseName?: string) => {
    const base =
      (baseName ?? datasetName ?? "generated-data").trim() || "generated-data";
  ...
  const handleDataGenerated = async (
    ...
    if (projectId === null || Number.isNaN(projectId)) {
      toast({
        ...
        description:
          "Invalid project context. Please reopen this page from a project.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `Detected ${
          splitInfo?.total_rows ?? data.length
        } row(s) in generated file.`,
      });
      ...
      toast({
        ...
        description: `Imported ${
          splitInfo?.total_rows ?? data.length
        } row(s) to ${project?.name || "project"}.`,
      });

      const fileIdsParam =
        importResponse.batch.file_ids &&
        importResponse.batch.file_ids.length > 0
          ? `&fileIds=${encodeURIComponent(
              JSON.stringify(importResponse.batch.file_ids)
            )}`
          : "";
      ...
      sonnerToast.error(error.message || "Failed to import generated data", {
        id: toastId,
      });
      toast({
        title: "Error",
  ...
  return (
    <div className="space-y-4 p-6">
      <DataGenerator onDataGenerated={handleDataGenerated} onBack={handleBack} />
      {importing && (
        <p className="text-sm text-muted-foreground">
          Importing generated data into {project?.name || "project"} and
          creating chunk batches...
        </p>
      )}
    </div>
  );

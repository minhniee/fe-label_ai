...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!topic || !apiKey) {
      return NextResponse.json({ success: false, error: "Topic and API key are required" }, { status: 400 })
    }

    console.log("[v0] Generating data with params:", { topic, rowCount, columns })
